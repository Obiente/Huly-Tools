              ...
              >
                <td class="px-6 py-4 whitespace-nowrap">
                  <div class="flex items-center">
                    <div
                      class="w-8 h-8 bg-blue-600 rounded-full flex items-center justify-center text-sm font-medium"
                    >
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                  <span
                    :class="statusClasses(user.confirmed)"
                    class="inline-flex px-2 py-1 text-xs font-semibold rounded-full"
                  >
...
<script setup lang="ts">
  ...
  async function saveUser() {
    ...
    try {
      ...
      if (response.ok) {
        showNotification(
          `User ${editingUser.value ? "updated" : "created"} successfully`,
          "success"
        );
        closeModal();
        await loadUsers();
        ...
        showNotification(
          `Failed to ${editingUser.value ? "update" : "create"} user`,
          "error"
        );
      }
    } catch (error) {
